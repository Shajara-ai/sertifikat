...
def process_mail_merge():
    ...
    for row_idx in range(3, row_count + 1):
        ...
        # Hitung jumlah penandatangan aktif
        signers_list = []
        if ttd1_nama:
        ...
        qr_shape_boxes = {}
        active_signers_count = len(signers_list)
            ...
            if shape.has_text_frame:
                ...
                for qr_ph in qr_placeholders:
                    if qr_ph in shape.text:
                        ...
                        if qr_ph == "{QR}":
                            is_valid_qr = True
                        elif qr_ph == "{QR1}" and active_signers_count >= 1:
                            is_valid_qr = True
                        elif qr_ph == "{QR2}" and active_signers_count >= 2:
                            is_valid_qr = True
                        elif qr_ph == "{QR3}" and active_signers_count >= 3:
                            is_valid_qr = True
    ...
    # 4. Generate database.js untuk website verifikasi
    ...
    # 5. Otomatisasi Git Push Khusus File database.js (Mengabaikan folder PDF)
    print("\n[OTOMASI GITHUB] Memulai sinkronisasi teks database ke GitHub...")
    try:
        os.chdir(BASE_DIR)
        
        # Hanya mendaftarkan file database.js yang di-upload ke repository online
        subprocess.run(["git", "add", "output/database.js"], check=True)
        subprocess.run(["git", "commit", "-m", "Otomatis update database sertifikat (Abaikan PDF)"], check=True)
        subprocess.run(["git", "push", "origin", "main"], check=True)
        
        print("-> [SUKSES] Sinkronisasi online database.js ke GitHub Pages berhasil dilakukan!")
    except Exception as git_error:
        print(f"-> [LOG GITHUB] Sinkronisasi otomatis dilewati / gagal. Detail: {git_error}")
        print("   Jalankan git push manual jika ingin sinkronisasi secara manual lewat terminal.")

    print("\n=== Proses Selesai! ===")
    print(f"Total sertifikat baru diproses: {new_certs_processed}")
    print(f"Semua file siap dicek secara lokal berada di: {OUTPUT_DIR}")
