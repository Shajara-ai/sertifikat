...
def process_mail_merge():
    ...
    for row_idx in range(3, row_count + 1):
        ...
        # Ganti teks placeholder dan cari posisi semua shape QR secara dinamis
        qr_placeholders = ["{QR}", "{QR1}", "{QR2}", "{QR3}"]
        qr_shape_boxes = {}
        
        # Hitung berapa jumlah penandatangan yang valid/diisi
            ...
            if shape.has_text_frame:
                ...
                for qr_ph in qr_placeholders:
                    if qr_ph in shape.text:
                        # Logika filter jumlah QR:
                        # Jika placeholder adalah {QR1} tapi penandatangan cuma 1, ini boleh lewat.
                        # Jika placeholder {QR2} tapi penandatangan cuma 1, ini akan dihapus (tidak dijadikan barcode).
                        is_valid_qr = False
                        if qr_ph == "{QR}":
                            is_valid_qr = True
                        ...
                        if is_valid_qr:
                            # Catat koordinatnya untuk ditempeli gambar QR Code nanti
                        ...
                        # Hapus shape placeholder teks asli dari slide PowerPoint
                        sp = shape._element
                        sp.getparent().remove(sp)
                        break

        # Tempel gambar QR Code verifikasi HANYA di lokasi yang valid
