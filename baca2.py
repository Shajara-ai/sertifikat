...
def load_local_database():
    ...
    if os.path.exists(EXCEL_PATH):
        try:
            ...
            for row in ws.iter_rows(min_row=3, values_only=True):  # Mulai baris 3 (data)
...
def decode_qr_from_image(image_path):
    """Membaca QR Code dari gambar menggunakan OpenCV (Mendukung Single & Multi QR)"""
    ...
    # 1. Coba deteksi banyak QR code (Multi QR) - Cocok jika ada 3 QR Code berdampingan
    try:
        retval, decoded_info, points, straight_qrcode = detector.detectAndDecodeMulti(img)
        if retval:
            # Ambil hasil decode pertama yang tidak kosong
    ...
    # 2. Fallback ke deteksi QR code tunggal
    try:
    ...
    # 3. Jika gagal, ubah ke grayscale dan tingkatkan ketajaman
    try:
        gray = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
        
        # Coba deteksi Multi QR pada gambar grayscale
        retval_gray, decoded_info_gray, _, _ = detector.detectAndDecodeMulti(gray)
        if retval_gray:
            for val in decoded_info_gray:
                if val and val.strip():
                    return val.strip()
                    
        # Coba deteksi Single QR pada gambar grayscale
        val_gray, _, _ = detector.detectAndDecode(gray)
        if val_gray and val_gray.strip():
            return val_gray.strip()
    except Exception:
        pass
...
def scan_pdf_for_qr(pdf_path):
    ...
    for page_idx in range(min(5, len(doc))):
        ...
        # Render halaman ke gambar dengan resolusi tinggi (zoom=3.0 untuk deteksi Multi QR lebih presisi)
        zoom = 3.0
    ...
    for pdf_path in pdf_files:
        ...
        if qr_url:
            ...
            if cert_id in db:
                ...
                # Tampilkan daftar penandatangan di terminal
                if "signers" in data and isinstance(data["signers"], list):
                    print("Penandatangan    :")
                    for idx, signer in enumerate(data["signers"]):
                        print(f"  {idx + 1}. {signer['name']} ({signer['role']})")
                
                # LANGSUNG BUKA LINK ONLINE GITHUB PAGES (Mengabaikan HTML lokal)
                if qr_url.startswith("http://") or qr_url.startswith("https://"):
                    open_in_browser(qr_url)
                else:
                    # Proteksi jika isi QR code hanya teks ID biasa, otomatis dibungkus ke URL GitHub
                    github_online_url = f"https://shajara-ai.github.io/sertifikat/?id={urllib.parse.quote(cert_id)}"
                    open_in_browser(github_online_url)
            else:
                print(f"Nomor Sertifikat : {cert_id}")
